...
- **Tracking routes are PUBLIC and **token-only**.** None of the three tracking routes (`/t/o/...`, `/t/c/...`, `/t/u/...`) use `signed` middleware. The reason is that `DOMDocument::saveHTML()` and Blade's `{{ }}` both HTML-entity-encode `&` to `&amp;` in href attributes, which corrupts the `&signature=...` query param whenever a user copy-pastes a URL from `storage/logs/laravel.log` or a permissive mail client doesn't decode entities. To avoid this entirely: the click URL carries its target as a **path segment** (`/t/c/{recipient}/{token}/{base64urlTarget}`), not a query param. Security comes from the per-recipient 40-char `tracking_token` (~240 bits of entropy, unguessable). The `t/u/*` POST route is still excluded from CSRF in `bootstrap/app.php` so `List-Unsubscribe-Post: List-Unsubscribe=One-Click` works from Gmail. **Do not re-add `signed` middleware to these routes** without also moving the signature out of the query string.

- **BYOK keys now persist for 14 days by default.** The `RunCampaignPipeline::handle()` `finally { wipe }` block was intentionally removed — the user's LLM key stays on the encrypted `api_key` column from the moment they create the campaign until either (a) the user manually clicks "Detener secuencia", (b) `api_key_retention_expires_at` passes and `WipeExpiredCampaignKeysCommand` picks it up on its hourly run, or (c) all follow-up recipients reach a terminal state. Retention is set at campaign creation in `CampaignController::store()` as `now()->addDays(config('services.roomie.followup_max_retention_days'))`. The earlier "wipe every run" invariant was rejected by the user because it forced a re-paste on every send. **Do not re-add the wipe** — the hard cap + manual stop + hourly sweep is the new security posture.

- **Follow-up escalation reuses `intensityGuidance()`.** `CampaignPipeline::regenerateForFollowup()` mutates `$this->aggressiveness` and `$this->persuasionPatterns` to `min(5, original + attempt - 1)` before calling `runCreativeFollowup()`, which reuses the same intensity helper as the initial run. **One creative per attempt, not per recipient**: the generated creative is cached in `campaigns.followup_variants[attempt]` (JSON) and reused for every recipient who lands on that attempt — saves ~90% of LLM calls in the loop. **The Auditor is NOT re-run** on follow-ups (25% LLM cost savings, coherence already validated on the original).
...
- **The Creativo agent writes editorial email bodies against a fixed design system.** `CampaignPipeline::creativeDesignGuide()` is the single source of truth for what a good `body_html` looks like — Georgia serif, the five-color Roomie palette, 20px lead paragraph, 17px/1.7 body, optional pull-quote with a 2px copper left border, optional 3-item list built with a `<table>` (for Outlook), sparkle dividers via the `&#10022;` Unicode glyph, and a Georgia italic sign-off. The same guide is threaded into **three** prompts: the initial `runCreative()`, the `runCreativeFollowup()` escalation, and the interactive `refineCreative()` that lets the user ship free-form instructions from `/campaigns/{id}` to re-generate the creative against the full original context. The refinement endpoint is `POST /campaigns/{id}/refine-creative` (web + API) and requires the campaign to still have its retained `api_key` — if it has been wiped, the button in the UI renders as a disabled note and the API returns `key_not_available`. The outer email shell (`resources/views/emails/campaign.blade.php`) is 640px, table-based, has an Outlook VML fallback for the rounded CTA, and deliberately stays in email-safe territory (no SVG, no remote fonts, no flex/grid). Do not re-introduce Inter / Fredoka fonts inside the email — mail clients won't load them and the fallbacks look wrong.

<x-layouts.app title="Inicio">
    {{-- Hero --}}
    <section class="max-w-3xl pt-6 pb-20">
        <p class="font-mono text-[11px] uppercase tracking-[0.18em] text-navy/45 mb-7">
            Roomie · Eurostars Hotel Company · Impacthon 2026
        </p>

        <h1 class="font-[Fredoka] font-semibold text-5xl md:text-[3.75rem] leading-[1.02] tracking-tight mb-7">
            Tú escribes el objetivo.<br>
            <span class="text-copper">Cuatro agentes</span> hacen el resto.
        </h1>

        <p class="text-lg text-navy/65 leading-relaxed max-w-xl mb-9">
        ...
        <div class="flex items-center gap-6">
            <a href="{{ route('campaigns.create') }}"
               class="inline-flex items-center gap-2 bg-navy text-cream pl-6 pr-5 py-3 rounded-full font-medium hover:bg-navy-light transition">
                Crear una campaña
                <svg class="w-3 h-3 text-copper" viewBox="0 0 24 24"><use href="#roomie-sparkle"/></svg>
            </a>
            <a href="{{ route('campaigns.index') }}" class="text-sm text-navy/60 hover:text-navy underline underline-offset-4 decoration-navy/20 hover:decoration-navy transition">
    ...
    <section class="border-t border-navy/15 pt-14 pb-4">
        <div class="grid grid-cols-12 gap-x-8 gap-y-8">
            ...
            <ol class="col-span-12 md:col-span-9 -mt-2">
                @foreach ([
                    ...
                    <li class="grid grid-cols-12 gap-4 py-7 border-b border-navy/10 last:border-0">
                        <span class="col-span-2 md:col-span-1 font-mono text-xs text-navy/35 pt-1.5">
                            0{{ $i + 1 }}
                        </span>
                        <h3 class="col-span-10 md:col-span-3 font-[Fredoka] font-semibold text-xl">
                            {{ $name }}
                        </h3>
                        <p class="col-start-3 md:col-start-5 col-span-10 md:col-span-8 text-navy/65 leading-relaxed">
                            {{ $desc }}
                        </p>
                    </li>
                @endforeach
